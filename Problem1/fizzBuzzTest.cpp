...
int main() {
    ...
    // Print the generated fizzbuzzResult for debugging
    cout << "Generated fizzbuzzResult:" << endl;
    for (int i = 0; i < 100; i++) {
        cout << fizzbuzzResult[i] << " ";
    }
    cout << endl << endl;

    // Check the expected vs generated values
    bool testPassed = true;
    for (int i = 0; i < 100; i++) {
        if (expectedResult[i] != fizzbuzzResult[i]) {
            cout << "Mismatch at index " << i << ": expected " << expectedResult[i] 
                 << " but got " << fizzbuzzResult[i] << endl;
            testPassed = false;
        }
    }
